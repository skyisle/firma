import type { Transaction, Holding } from '@firma/db';
...
export const aggregateHoldings = (txns: Transaction[]): Map<string, Holding> => {
  ...
  for (const t of sorted) {
    const h = map.get(t.ticker) ?? { ticker: t.ticker, shares: 0, costShares: 0, totalCost: 0 };
    ...
    if (t.type === 'buy') {
      ...
      // Proportional reduction keeps average cost per share constant
